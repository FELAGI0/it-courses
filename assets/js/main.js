    ...
    <nav class="navbar" id="navbar">
      <div class="container nav-inner">
        ...
        <div class="nav-actions">
          <button class="btn-icon theme-toggle" id="themeToggle" title="Переключить тему">
            ${ICONS.moon}
          </button>
          ...
          ${user
            ? `<a href="${ROOT}${PAGES}profile.html" class="btn btn-outline nav-profile">
                <span class="nav-profile__avatar">${ICONS.user}</span> ${user.name.split(" ")[0]}
               </a>`
    ...
    <footer class="footer">
      <div class="container">
        ...
        <div class="footer-bottom">
          <p>&copy; 2026 ITКурсы. Все права защищены.</p>
          <p>Сделано с вниманием к качеству</p>
        </div>
      </div>
    </footer>
  `;
    ...
    <div class="course-card animate-on-scroll" data-id="${course.id}">
      <a href="${courseUrl}" class="card-image-link">
        <div class="card-img" style="background: linear-gradient(135deg, ${course.color}15, ${course.color}08)">
          ...
          <button class="wishlist-btn ${inWL ? "active" : ""}" data-id="${course.id}" title="В избранное" aria-label="В избранное">
            ${inWL ? ICONS.heart : ICONS.heartOutline}
          </button>
        </div>
      </a>
      ...
      <div class="card-body">
        ...
        <div class="card-meta">
          <span>${ICONS.book} ${course.lessons} уроков</span>
        ...
        <div class="card-instructor">
          ${ICONS.teacher} ${course.instructor}
        </div>
        ...
        <div class="card-footer">
          ...
          <button class="btn ${inCart ? "btn-success" : "btn-primary"} btn-sm add-to-cart" data-id="${course.id}">
            ${inCart ? `${ICONS.check} В корзине` : "В корзину"}
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="teacher-card animate-on-scroll">
      ...
      <div class="teacher-stats">
        <div class="teacher-stat">
          <span class="teacher-stat-num">${teacher.students.toLocaleString("ru-RU")}</span>
          <span class="teacher-stat-label">Студентов</span>
        </div>
        <div class="teacher-stat">
          <span class="teacher-stat-num">${teacher.courses}</span>
          <span class="teacher-stat-label">Курсов</span>
        </div>
        <div class="teacher-stat">
          <span class="teacher-stat-num">${teacher.rating}</span>
...
function trigramSearch(query, courses) {
  ...
  const scored = courses.map(course => {
    ...
    // Считаем пересечение триграмм
    let matches = 0;
    queryTrigrams.forEach(tg => {
      if (courseTrigrams.has(tg)) matches++;
    });

    const score = queryTrigrams.size > 0 ? matches / queryTrigrams.size : 0;

    return { course, score };
  });

  // Сначала точные совпадения подстроки, потом по триграммам
  ...
  const trigramResults = scored
    .filter(s => !exactIds.has(s.course.id) && s.score >= 0.3)
